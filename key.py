...
class KeyloggerDetectionApp:
    def __init__(self, root):
        ...
        self.buttons = [
            ...
            ("Monitor Processes", self.run_in_thread(self.monitor_processes), "🔁", "Alt-m"),
    ...
    def honeyid_simulation(self):
        ...
        try:
            for i in range(5):
                bogus_key = chr(65 + i)
                pyautogui.press(bogus_key.lower())
    ...
    def monitor_processes(self):
        self.clear_display()
        self.log("Live Process Monitor (updates every 2 seconds):\n")
        def monitor():
            while True:
                self.text_area.delete(1.0, tk.END)
                self.text_area.insert(tk.END, f"{'PID':<8}{'Name':<25}{'CPU%':<10}{'Memory%':<10}\n")
                self.text_area.insert(tk.END, f"{'-'*60}\n")
                for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
                    try:
                        self.text_area.insert(tk.END, f"{proc.info['pid']:<8}{proc.info['name'][:24]:<25}{proc.info['cpu_percent']:<10.1f}{proc.info['memory_percent']:<10.1f}\n")
                    except:
                        continue
                time.sleep(2)
